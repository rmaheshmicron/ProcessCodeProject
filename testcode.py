...
def load_data_from_sql_server():
    ...
    if "sql_server" in st.secrets and "sql_database" in st.secrets:
        ...
        st.sidebar.subheader("SQL Server Authentication")
    ...
    if not (server and database):
        st.sidebar.warning("Please provide SQL Server connection details.")
        return sql_data
    ...
    try:
        ...
        if username and password:
            ...
            connection_string = f"mssql+pyodbc://{urllib.parse.quote_plus(username)}:{urllib.parse.quote_plus(password)}@{server}/{database}?driver=ODBC+Driver+17+for+SQL+Server"
        else:
        ...
        # Query ModuleBOM_59only
        query_59only = "SELECT * FROM ModuleBOM_59only"
        sql_data['module_bom_59only_df'] = pd.read_sql(query_59only, engine)
        st.sidebar.success(f"Retrieved {len(sql_data['module_bom_59only_df'])} records from ModuleBOM_59only")
        
        # Query ModuleBOM_Simple
        query_simple = "SELECT * FROM ModuleBOM_Simple"
        sql_data['module_bom_simple_df'] = pd.read_sql(query_simple, engine)
        st.sidebar.success(f"Retrieved {len(sql_data['module_bom_simple_df'])} records from ModuleBOM_Simple")
        
        # Close the connection
        engine.dispose()
        
    except Exception as e:
        st.sidebar.error(f"Error connecting to SQL Server: {str(e)}")
        
        with st.sidebar.expander("SQL Server Error Details", expanded=False):
            st.write(str(e))
            import traceback
            st.write(traceback.format_exc())
            
            st.write("Common solutions:")
            st.write("1. Verify SQL Server is accessible from your network")
            st.write("2. Check if ODBC Driver 17 for SQL Server is installed")
            st.write("3. Verify username and password are correct")
            st.write("4. Ensure the database and tables exist")
            st.write("5. Check firewall settings and SQL Server configuration")
...
def load_combined_data():
    """Load data from both SharePoint and SQL Server"""
    # Load SharePoint data
    sharepoint_data = load_data_from_sharepoint()
    ...
    combined_data = {
        'component_validations_df': sharepoint_data['component_validations_df'],
        'module_validation_df': sharepoint_data['module_validation_df'],
...
def get_process_code_from_sql(mpn, module_bom_59only_df, module_bom_simple_df):
    """
    Look up Process Code from SQL databases using MPN (Material Description)
    """
    try:
        if not mpn:
        ...
        # Search in ModuleBOM_59only first (has Process Code directly)
        if not module_bom_59only_df.empty:
            # Check if the required columns exist
            if 'Material Description' in module_bom_59only_df.columns and 'Process Code' in module_bom_59only_df.columns:
                # Search for exact match first
                exact_match = module_bom_59only_df[
                    module_bom_59only_df['Material Description'].str.lower() == mpn.lower()
                ]
                ...
                if not exact_match.empty:
                    for _, row in exact_match.iterrows():
                        results.append({
                            ...
                            'BOM Number': row.get('BOM Number', ''),
                            'Process Code': row.get('Process Code', ''),
                            'Match Type': 'Exact'
                        })
                
                # If no exact match, try partial match
                if not results:
                    partial_match = module_bom_59only_df[
                        module_bom_59only_df['Material Description'].str.contains(mpn, case=False, na=False)
                    ]
        ...
        # Search in ModuleBOM_Simple for additional BOM information
        if not module_bom_simple_df.empty:
            if 'Material_Description' in module_bom_simple_df.columns:
                # Search for exact match first
                exact_match_simple = module_bom_simple_df[
                    module_bom_simple_df['Material_Description'].str.lower() == mpn.lower()
                ]
                
                if not exact_match_simple.empty:
                    for _, row in exact_match_simple.iterrows():
                        # Check if we already have this BOM from the 59only table
                        bom_number = row.get('BOM', '')
                        existing_bom = any(r['BOM Number'] == bom_number for r in results)
                        
                        if not existing_bom:
                            results.append({
                                'Source': 'ModuleBOM_Simple',
                                'Material Description': row.get('Material_Description', ''),
                                'BOM Number': bom_number,
                                'Process Code': 'Not Available in Simple Table',
                                'Match Type': 'Exact'
                            })
                
                # If no exact match, try partial match
                if not any(r['Source'] == 'ModuleBOM_Simple' for r in results):
                    partial_match_simple = module_bom_simple_df[
                        module_bom_simple_df['Material_Description'].str.contains(mpn, case=False, na=False)
                    ]
                    
                    if not partial_match_simple.empty:
                        for _, row in partial_match_simple.iterrows():
                            bom_number = row.get('BOM', '')
                            existing_bom = any(r['BOM Number'] == bom_number for r in results)
        ...
        # Try to cross-reference BOM numbers between tables to find Process Codes
        for result in results:
            if result['Process Code'] == 'Not Available in Simple Table' and result['BOM Number']:
                # Look for this BOM in the 59only table
                bom_match = module_bom_59only_df[
                    module_bom_59only_df['BOM Number'] == result['BOM Number']
                ] if not module_bom_59only_df.empty and 'BOM Number' in module_bom_59only_df.columns else pd.DataFrame()
                
                if not bom_match.empty:
                    process_code = bom_match.iloc[0].get('Process Code', '')
                    if process_code:
                        result['Process Code'] = process_code
                        result['Source'] = 'Cross-referenced from ModuleBOM_59only'
        
        if not results:
            return f"No matching records found for MPN: {mpn}", None
        
        # Convert results to DataFrame
        results_df = pd.DataFrame(results)
        
        # Sort by match type (exact matches first) and source
        results_df['Sort_Order'] = results_df['Match Type'].map({'Exact': 1, 'Partial': 2})
        results_df = results_df.sort_values(['Sort_Order', 'Source']).drop('Sort_Order', axis=1)
        
        return "Success", results_df
    
    except Exception as e:
        return f"Error searching SQL databases: {e}", None

def get_available_mpns(module_bom_59only_df, module_bom_simple_df):
    """
    Get list of available MPNs from both SQL tables
    """
    mpns = set()
    
    try:
        # Get MPNs from ModuleBOM_59only
        if not module_bom_59only_df.empty and 'Material Description' in module_bom_59only_df.columns:
            mpns.update(module_bom_59only_df['Material Description'].dropna().unique())
        
        # Get MPNs from ModuleBOM_Simple
        if not module_bom_simple_df.empty and 'Material_Description' in module_bom_simple_df.columns:
            mpns.update(module_bom_simple_df['Material_Description'].dropna().unique())
        
        # Clean and sort the MPNs
        cleaned_mpns = [mpn.strip() for mpn in mpns if mpn and str(mpn).strip()]
        return sorted(cleaned_mpns)
    
    except Exception as e:
        st.error(f"Error getting available MPNs: {e}")
        return []

def search_mpn_in_sql(search_term, module_bom_59only_df, module_bom_simple_df):
    """
    Search for MPNs that contain the search term
    """
    matching_mpns = []
    ...
    try:
        ...
        if not module_bom_59only_df.empty and 'Material Description' in module_bom_59only_df.columns:
            matches = module_bom_59only_df[
                module_bom_59only_df['Material Description'].str.contains(search_term, case=False, na=False)
        ...
        if not module_bom_simple_df.empty and 'Material_Description' in module_bom_simple_df.columns:
            matches = module_bom_simple_df[
                module_bom_simple_df['Material_Description'].str.contains(search_term, case=False, na=False)
            ]['Material_Description'].unique()
            matching_mpns.extend(matches)
        
        # Remove duplicates and sort
        unique_mpns = list(set(matching_mpns))
        return sorted(unique_mpns)
...
def analyze_sql_data(module_bom_59only_df, module_bom_simple_df):
    """Analyze and display SQL Server data structure"""
    st.subheader("SQL Server Data Analysis")
    
    if not module_bom_59only_df.empty:
        with st.expander("ModuleBOM_59only Table Structure", expanded=False):
            st.write(f"**Rows:** {len(module_bom_59only_df)}")
            st.write(f"**Columns:** {len(module_bom_59only_df.columns)}")
            st.write("**Column Names:**")
            st.write(list(module_bom_59only_df.columns))
            
            st.write("**Sample Data:**")
            st.dataframe(module_bom_59only_df.head())
            
            st.write("**Data Types:**")
            st.write(module_bom_59only_df.dtypes)
    
    if not module_bom_simple_df.empty:
        with st.expander("ModuleBOM_Simple Table Structure", expanded=False):
            st.write(f"**Rows:** {len(module_bom_simple_df)}")
            st.write(f"**Columns:** {len(module_bom_simple_df.columns)}")
            st.write("**Column Names:**")
            st.write(list(module_bom_simple_df.columns))
            
            st.write("**Sample Data:**")
            st.dataframe(module_bom_simple_df.head())
            
            st.write("**Data Types:**")
            st.write(module_bom_simple_df.dtypes)
...
def main():
    ...
    if st.sidebar.button("Refresh Data"):
        if hasattr(load_combined_data, 'clear'):
    ...
    module_bom_59only_df = data['module_bom_59only_df']
    module_bom_simple_df = data['module_bom_simple_df']
    ...
    # Add SQL data analysis in sidebar
    if not module_bom_59only_df.empty or not module_bom_simple_df.empty:
        with st.sidebar.expander("SQL Data Status", expanded=False):
            if not module_bom_59only_df.empty:
                st.write(f"✅ ModuleBOM_59only: {len(module_bom_59only_df)} records")
            else:
                st.write("❌ ModuleBOM_59only: No data")
            
            if not module_bom_simple_df.empty:
                st.write(f"✅ ModuleBOM_Simple: {len(module_bom_simple_df)} records")
            else:
                st.write("❌ ModuleBOM_Simple: No data")
    
    tab1, tab2, tab3 = st.tabs(["Module Process Code Lookup", "Module Process Code Generator", "MPN Process Code Lookup"])
