...
def load_data_from_sharepoint():
    ...
    if "sharepoint_username" in st.secrets and "sharepoint_password" in st.secrets:
        ...
        # Return empty data if no credentials in secrets
        return data
...
def get_sharepoint_credentials():
    """Get SharePoint credentials from user input or secrets"""
    if "sharepoint_username" in st.secrets and "sharepoint_password" in st.secrets:
        return st.secrets["sharepoint_username"], st.secrets["sharepoint_password"]
    else:
...
def main():
    ...
    # Handle SharePoint authentication UI
    if component_validations_df.empty:
        username, password = get_sharepoint_credentials()
        if username and password:
            st.sidebar.warning("SharePoint credentials provided. Click 'Refresh Data' to load data.")
        else:
            st.sidebar.warning("Please provide SharePoint credentials to load data.")
    else:
        st.sidebar.success(f"SharePoint data loaded: {len(component_validations_df)} component records")
    
    # Load SQL Server data (handled separately to avoid caching issues with widgets)
    ...
    tab1, tab2, tab3 = st.tabs(["Component Process Code", "Module Process Code", "MPN Lookup"])
    
    with tab1:
        st.write("Generate process codes for individual components:")
        
        col1, col2 = st.columns(2)
        
        with col1:
            segment = st.selectbox("Segment", options=predefined_options['segment'], key="comp_segment")
            
            supplier_options = get_filtered_options(component_validations_df, 'Supplier', segment=segment)
            if not supplier_options:
                supplier_options = predefined_options['supplier']
            supplier = st.selectbox("Supplier", options=supplier_options, key="comp_supplier")
            
            component_type_options = get_filtered_options(component_validations_df, 'Component_Type', segment=segment, supplier=supplier)
            if not component_type_options:
                if segment and segment.lower() == 'client':
                    component_type_options = ["CKD", "PMIC", "SPD/Hub", "Inductor", "Voltage Regulator"]
                else:
                    component_type_options = predefined_options['component_type']
            component_type = st.selectbox("Component Type", options=component_type_options, key="comp_type")
        
        with col2:
            component_gen_options = get_filtered_options(component_validations_df, 'Component_Generation', segment=segment, supplier=supplier, component_type=component_type)
            if not component_gen_options:
                component_gen_options = predefined_options['component_generation']
            component_gen = st.selectbox("Component Generation", options=component_gen_options, key="comp_gen")
            
            revision_options = get_filtered_options(component_validations_df, 'Revision', segment=segment, supplier=supplier)
            if not revision_options:
                revision_options = predefined_options['revision']
            revision = st.selectbox("Revision", options=revision_options, key="comp_revision")
        
        if st.button("Generate Component Process Code"):
            if not all([segment, supplier, component_gen, revision, component_type]):
                st.error("Please fill in all fields")
            else:
                process_code, component_type_result, code_details = get_component_process_code(
                ...
                if code_details is not None and not code_details.empty:
                    st.success(f"Process Code: **{process_code}**")
    ...
    with tab2:
        st.write("Generate process codes for complete modules:")
        
        col1, col2 = st.columns(2)
        
        with col1:
            module_segment = st.selectbox("Segment", options=predefined_options['segment'], key="mod_segment")
            
            st.subheader("Component Process Codes")
            pmic = st.text_input("PMIC Process Code", key="pmic_code", help="Required for all modules")
            spd_hub = st.text_input("SPD/Hub Process Code", key="spd_hub_code", help="Required for all modules")
            
            if module_segment and module_segment.lower() == 'server':
                temp_sensor = st.text_input("Temp Sensor Process Code", key="temp_sensor_code", help="Required for server modules")
                rcd_mrcd = st.text_input("RCD/MRCD Process Code", key="rcd_mrcd_code", help="Required for server modules")
                data_buffer = st.text_input("Data Buffer Process Code", key="data_buffer_code", help="Optional for server modules")
            elif module_segment and module_segment.lower() == 'client':
                temp_sensor = st.text_input("CKD Process Code", key="ckd_code", help="Optional for client modules")
                rcd_mrcd = ""
                data_buffer = ""
            else:
                temp_sensor = ""
        ...
        if st.button("Generate Module Process Code"):
            if not module_segment:
                ...
                module_process_code = get_module_process_code(pmic, spd_hub, temp_sensor, rcd_mrcd, data_buffer, module_segment)
                
                if "required" in module_process_code.lower() or "error" in module_process_code.lower() or "invalid" in module_process_code.lower():
                    ...
                    # Look up parts for each component
                    st.subheader("Component Lookup")
                    lookup_result = lookup_parts_by_process_code(module_process_code, component_validations_df)
                    
                    if isinstance(lookup_result, pd.DataFrame):
                        st.dataframe(lookup_result, height=400)
                    else:
                        st.warning(lookup_result)
