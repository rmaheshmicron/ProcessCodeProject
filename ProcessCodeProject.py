...
def get_sql_connection_params():
    ...
    if "sql_database" in st.secrets:
        ...
        database = st.sidebar.text_input(
            ...
            key="sql_database_connection_params",  # Changed from "sql_database_main"
        ...
        username = st.sidebar.text_input(
            ...
            key="sql_username_connection_params",  # Changed from "sql_username_main"
        ...
        password = st.sidebar.text_input(
            ...
            key="sql_password_connection_params",  # Changed from "sql_password_main"
                    ...
                    if test_value and test_value[0] == 1:
                        ...
                        for table in tables_to_check:
                            try:
                                ...
                                st.sidebar.warning(f"{table}: {str(table_error)}")
...
def test_sql_connection_detailed():
    ...
    if st.sidebar.button("Test Connection", key="test_sql_conn_detailed"):  # Changed from "test_sql_conn_main"
        ...
        with st.sidebar:
            with st.spinner("Testing connection to remote SQL Server..."):
                ...
                for timeout in timeout_values:
                    st.info(f"Trying with {timeout}s timeout...")
                    ...
                    try:
                        ...
                        if timeout == timeout_values[-1]:  # Last timeout attempt
                            ...
                            st.warning(f"{timeout}s timeout failed, trying longer timeout...")
                        continue
...
def check_odbc_drivers():
    """Check available ODBC drivers and provide installation guidance"""
    try:
        import pyodbc
        drivers = pyodbc.drivers()
        
        st.sidebar.subheader("ODBC Driver Status")
        
        # Check for SQL Server drivers
        sql_drivers = [d for d in drivers if 'SQL Server' in d]
        
        if sql_drivers:
            st.sidebar.success("✅ SQL Server ODBC drivers found:")
            for driver in sql_drivers:
                st.sidebar.write(f"  • {driver}")
        else:
            st.sidebar.error("❌ No SQL Server ODBC drivers found")
            
            with st.sidebar.expander("📥 Install ODBC Drivers", expanded=True):
                st.write("""
                **For Windows:**
                
                1. **Download Microsoft ODBC Driver 17 for SQL Server:**
                   - Go to: https://docs.microsoft.com/en-us/sql/connect/odbc/download-odbc-driver-for-sql-server
                   - Download the appropriate version (x64 for 64-bit systems)
                
                2. **Run the installer as Administrator**
                
                3. **Alternative - Use winget (if available):**
                ```
        ...
        st.sidebar.error("pyodbc not installed. Install with: pip install pyodbc")
        return []
    except Exception as e:
        st.sidebar.warning(f"Could not check ODBC drivers: {e}")
        return []
...
def main():
    ...
    if st.sidebar.button("Refresh Data", key="refresh_data_main_unique"):  # Made key more unique
        # Clear cached data
