...
def get_rest_api_connection_params():
    ...
    # Otherwise get from sidebar with unique keys
    st.sidebar.subheader("REST API Connection")
    
    # Use session state to maintain values and avoid duplicate keys
    if 'rest_api_base_url' not in st.session_state:
        st.session_state.rest_api_base_url = 'http://localhost:8000'
    if 'rest_api_username' not in st.session_state:
        st.session_state.rest_api_username = 'admin'
    if 'rest_api_password' not in st.session_state:
        st.session_state.rest_api_password = 'MicronPC123'
    if 'rest_api_timeout' not in st.session_state:
        st.session_state.rest_api_timeout = 30
    
    base_url = st.sidebar.text_input(
        "API Base URL", 
        value=st.session_state.rest_api_base_url,
        help="Base URL for the REST API endpoint"
    )
    
    username = st.sidebar.text_input(
        "Username", 
        value=st.session_state.rest_api_username,
        help="API username"
    )
    ...
    password = st.sidebar.text_input(
        "Password", 
        type="password",
        value=st.session_state.rest_api_password,
        help="API password"
    )
    
    timeout = st.sidebar.number_input(
        "Timeout (seconds)", 
        value=st.session_state.rest_api_timeout,
        min_value=5,
        max_value=300
    )
    
    # Update session state
    st.session_state.rest_api_base_url = base_url
    st.session_state.rest_api_username = username
    st.session_state.rest_api_password = password
    st.session_state.rest_api_timeout = timeout
