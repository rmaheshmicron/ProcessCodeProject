...
def get_rest_api_connection_params():
    ...
    # Check if we already have the values in session state (to avoid duplicate widgets)
    if all(key in st.session_state for key in ["rest_api_base_url", "rest_api_username", "rest_api_password", "rest_api_timeout"]):
        return (
            st.session_state["rest_api_base_url"],
            st.session_state["rest_api_username"],
            st.session_state["rest_api_password"],
            st.session_state["rest_api_timeout"]
        )
    ...
    if "rest_api_widgets_created" not in st.session_state:
        ...
        return (
            ...
            st.session_state.get("rest_api_timeout", 30)
        )
...
def main():
    ...
    if st.sidebar.button("Refresh Data", key="refresh_data_main_unique"):
        ...
        # Clear session state for connection parameters AND widget creation flag
