...
def get_rest_api_connection_params():
    ...
    base_url = st.sidebar.text_input(
        "API Base URL", 
        value=st.session_state.get('rest_api_base_url', 'http://localhost:8000'),  # Fixed: use .get() method
    ...
    username = st.sidebar.text_input(
        "Username", 
        value="admin",  # Changed from "ProcessCodeAdmin" to "admin"
    ...
    timeout = st.sidebar.number_input(
        "Timeout (seconds)", 
        value=st.session_state.get('rest_api_timeout', 30),  # Fixed: use .get() method
