...
def get_rest_api_connection_params():
    ...
    base_url = st.sidebar.text_input(
        "API Base URL", 
        value=st.session_state.get('rest_api_base_url', 'https://your-api-server.com/api'),
    ...
    username = st.sidebar.text_input(
        "Username", 
        value=st.session_state.get('rest_api_username', ''),
...
def load_data_from_rest_api_cached(base_url, username, password, timeout):
    """Load data using REST API with better error handling"""
    data = {
    ...
    try:
        # Query ModuleBOM_Simple endpoint
        try:
            st.sidebar.info("Loading ModuleBOM_Simple...")
            
            # Get all data from the endpoint
            data_url = f"{api_base_url.rstrip('/')}/modulebom-simple"
            ...
            if response.status_code == 200:
                ...
                if records:
                    ...
                    module_bom_simple_df = pd.DataFrame(records)
                ...
                st.sidebar.error(f"ModuleBOM_Simple: HTTP {response.status_code} - {response.text}")
                
        except Exception as e:
            st.sidebar.error(f"ModuleBOM_Simple: {str(e)}")
        
    except Exception as e:
        st.sidebar.error(f"REST API connection error: {e}")
...
def search_mpn_optimized_rest_api(search_term, base_url, username, password, timeout):
    """Search for MPNs using REST API for better performance"""
    session_result = create_rest_api_session(base_url, username, password, timeout)
    
    if not session_result or not search_term:
        return []
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Handle different response formats
            if isinstance(json_data, list):
                matching_mpns = json_data
            elif isinstance(json_data, dict):
                matching_mpns = json_data.get('data', json_data.get('results', []))
            else:
                matching_mpns = []
            
            # Clean and sort results
            cleaned_mpns = []
            for mpn in matching_mpns:
                if isinstance(mpn, dict):
                    mpn_value = mpn.get('Material_Description', '')
                else:
                    mpn_value = str(mpn)
                
                if mpn_value and mpn_value.strip() and mpn_value.lower() != 'nan':
                    cleaned_mpns.append(mpn_value.strip())
            
            return sorted(list(set(cleaned_mpns)))
        else:
...
def get_process_code_optimized_rest_api(mpn, base_url, username, password, timeout):
    """Look up process code using REST API with optimized query"""
    session_result = create_rest_api_session(base_url, username, password, timeout)
    
    if not session_result or not mpn:
        return "No MPN provided", None
    
    session, api_base_url = session_result
    
    try:
        # Lookup endpoint with MPN parameter
        ...
        if response.status_code == 200:
            ...
            if isinstance(json_data, list):
                ...
                results = json_data.get('data', json_data.get('results', []))
            else:
                results = []
...
def search_mpn_in_rest_api(search_term, module_bom_simple_df):
    """Search for MPNs containing the search term in REST API data"""
    matching_mpns = []
...
def get_process_code_from_rest_api(mpn, module_bom_simple_df):
    """Look up process code for a given MPN in REST API data"""
    try:
        results = []
