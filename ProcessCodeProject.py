...
@st.cache_resource
def get_sql_engine():
    """Create a cached SQL engine with connection pooling"""
    ...
    try:
        ...
        st.sidebar.success("✅ SQL Server connection established")
        return engine
        
    except Exception as e:
        st.sidebar.error(f"❌ SQL Server connection failed: {e}")
        return None

@st.cache_data(ttl=3600)
def load_data_from_sql_server_sqlalchemy():
    """Load data using SQLAlchemy with better error handling and performance"""
    ...
    try:
        with engine.connect() as conn:
            # Query ModuleBOM_59only table with error handling
            try:
                ...
                st.sidebar.warning(f"⚠️ ModuleBOM_59only: {str(e)}")
            
            # Query ModuleBOM_Simple table with error handling
            try:
                ...
                st.sidebar.warning(f"⚠️ ModuleBOM_Simple: {str(e)}")
                
    except Exception as e:
        st.sidebar.error(f"❌ Database connection error: {e}")
        # Clear the cached engine if there's a connection error
        if hasattr(get_sql_engine, 'clear'):
            get_sql_engine.clear()
    
    return data

def search_mpn_optimized(search_term, engine):
...
def get_process_code_optimized(mpn, engine):
...
def load_data_from_sql_server():
    """Wrapper function that calls SQLAlchemy version"""
    return load_data_from_sql_server_sqlalchemy()

def analyze_sql_data_optimized(engine):
...
def main():
    ...
    # Remove the duplicate button - keep only one
