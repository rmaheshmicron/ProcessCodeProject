...
def create_sql_engine(server, database, username, password):
    """Create SQL engine with improved error handling for remote SQL Server"""
    ...
    try:
        # Try different driver options in order of preference
        drivers = [
        ...
        for driver in drivers:
            try:
                ...
                # SQL Server authentication connection string for remote server
                connection_string = (
                ...
                if "login failed" in error_msg.lower():
                    ...
                    st.sidebar.error("🌐 Network/Server issue - Check server name and network connectivity")
                elif "timeout" in error_msg.lower():
                    st.sidebar.error("⏱️ Connection timeout - Server may be slow or unreachable")
        ...
        with st.sidebar.expander("🔧 Troubleshooting Help", expanded=True):
            st.write(f"""
            **Connection Details:**
            - Server: `{server}`
            - Database: `{database}`
            ...
            **Common Issues:**
            ...
            1. **Credentials**: Verify username/password are correct
            
            2. **Network Access**: 
               - VPN connection may be required
               - Firewall may be blocking port 1433
               - Server may only accept connections from specific IPs
            
            3. **Database Access**: 
               - User must have permissions to the specified database
            ...
            5. **SSL/TLS Issues**: 
               - Try adding `TrustServerCertificate=yes` to connection
            """)
        
        return None

def test_sql_connection_detailed():
    """Enhanced connection test function for remote SQL Server"""
    st.sidebar.subheader("🔍 Test SQL Connection")
    
    if st.sidebar.button("Test Connection", key="test_sql_conn"):
